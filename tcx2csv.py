...
#libxslt
	...
	for file in glob.glob("*.tcx"):
		...
		with open(file_root+'.csv','w',newline='') as csvfile:
			...
			long0=False
			dist=0
			for tp in root.findall('.//'+ns+'Trackpoint'):
				try:
					tp_time=tp.find(ns+'Time').text
				except:
					tp_time=''
				try:
					tp_alt=tp.find(ns+'AltitudeMeters').text
				except:
					tp_alt=''
				try:
					tp_lat=tp.find(ns+'Position/'+ns+'LatitudeDegrees').text
				except:
					tp_lat=''
				try:
					tp_long=tp.find(ns+'Position/'+ns+'LongitudeDegrees').text
				except:
					tp_long=''
				try:
					tp_hr=tp.find(ns+'HeartRateBpm/'+ns+'Value').text
				except:
					tp_hr=''
				if len(tp_time) > 0:
				...
				if len(tp_lat) > 0 and len(tp_long) > 0:
					...
					if not lat0:
						...
						if not (lat0==lat and long0==long):
